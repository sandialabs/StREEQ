...
class MPI(ParallelEvaluator):
    ...
    def evaluate(self, model_runs, QOI, subset, initial_fits):
        ranked_model_runs = self.get_ranked_model_runs(model_runs)
        ranked_model_fits = []
        ...
        for q, model_run in enumerate(ranked_model_runs):
            ranked_model_fits.append(fitting_function(model_run))
            new_completion = int(100 * q / (len(model_runs) / comm.size))
            if int(new_completion/10) > int(completion/10):
                completion = new_completion
                log.info(f"    completed {completion}% of model fits")
        model_fits = self.assemble_ranked_model_fits(ranked_model_fits)
